...
class NFLBoard(BoardBase):
    ...
    def _render_favorite_teams(self, favorite_teams: dict, favorite_team_games: list, favorite_game_team_ids: set, all_available_games: list = None, team_schedule_games: dict = None):
        ...
        for team_id in self.team_ids:
            ...
            if team_id in favorite_game_team_ids:
                ...
                if team_game:
                    debug.info(f"Showing game for favorite team {team_id}")
                ...
                if team_id in favorite_teams:
                    ...
                    debug.info(f"Showing team summary for favorite team {team_id} (no game today)")
                    ...
                    debug.info(f"Using {len(team_schedule)} schedule games for team {team_id} summary")
    ...
    def _render_other_games(self, other_games: list):
        ...
        """
        debug.info(f"Showing {len(other_games)} other games")
    ...
    def _fetch_snapshot(self):
        ...
        """
        try:
            snapshot = self._fetch_data_using_simple_methods()
            return snapshot
    ...
    def _fetch_data_using_simple_methods(self) -> dict:
        debug.info(f"Fetching NFL data for {len(self.team_ids)} favorite teams")

        # Step 1: Get current games from scoreboard
        current_games = self.api_client.get_scoreboard()
        debug.info(f"Fetched {len(current_games)} games from current scoreboard")
        ...
        for team_id in self.team_ids:
            try:
                ...
                if team:
                    ...
                    debug.info(f"Loaded {len(schedule)} schedule games for team {team_id}")
        ...
        if self.show_previous_games_until:
            if self._should_show_previous_games(self.show_previous_games_until):
                ...
                debug.info(f"Added {len(previous_games)} previous games")
            else:
                ...
                debug.info(f"Filtered games based on time cutoff")
